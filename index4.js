...
app.get("/get",async(req,res)=>{
   ...
   const listofsub = [];
    ...
    $(element).find('ul.list-group.list-group-flush a').each((index, el) => {
      ...
      listofsub.push({subject:title.trim(),data:lp});
...
//   res.json(subjectList);
  ...
  const jsonData = JSON.stringify(listofsub, null, 2);
  
  // Replace 'papers.json' with the desired file name and path
  fs.writeFile(`./notes/${req.body.sem}`, jsonData, 'utf8', (err) => {
    if (err) {
